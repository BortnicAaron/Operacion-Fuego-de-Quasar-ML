...
exports.create = (name, x, y) => {
  ...
  const bodyRes = Promise.all([
    positionPromise,
    satellitePromise
  ])
  .then(([position,satellite])=>{
...
exports.createList = (satellites) => {
  ...
  if(satellitesVer) return Promise.reject(new Error('Faltan datos requeridos!'))
...
// Find a single Tutorial with an id
exports.findOne = (name,distance,message) => {
  if(typeof name !== 'string') return Promise.reject(new Error("Error name !== 'string'"))
